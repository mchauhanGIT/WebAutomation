...
Scenario Outline: Validate the user is able to create new record for organization in VAST system when no customer is retreived from the system
	...
	And user clicks on "Organization_type" and selects "<Organization type>" from "available_Organization_types"
	And user clicks on "Registered_state" and selects "<Registered state>" from "available_Registered_states"
	And user enters text "<Street address>" in textbox "Street_address"
	And user enters text "<City>" in textbox "City"
	And user clicks on "State" and selects "<State>" from "available_States"
	And user enters text "<Zip code>" in textbox "Zip_code"	
	And user clicks on button "Add_Phone_number"	
	And user clicks on "Phone_type" and selects "<Phone type>" from "available_Phone_types"
	...
	And user clicks on "ID_type" and selects "<ID type>" from "available_ID_types"	
	And user enters text "<Number>" in textbox "Number"	
	And user enters text "<Issuing authority>" in textbox "Issuing_authority"
	And user clicks on "ID_State" and selects "<ID state>" from "available_ID_States"
	...
	And user clicks on "Officer_code" and selects "<Officer code>" from "available_officer_codes"
	And user clicks on "Branch" and selects "<Branch>" from "available_Branches"	
	And user clicks on button "Next"
	And user is on "Capture_relationships_screen"  
	And user clicks on button "Add_Relationships"
	And user clicks on "Relation_type" and selects "<Relation type>" from "available_Relation_types"	
	...
	|	114464611		|	VAST Entity	|	Corporation			|	Oklahoma 		 |	1234 Main St	|	Tulsa	|	Oklahoma	|	12345		|	Cell		|	987-654-3210	|	abc@abc.com		|	Articles of Incorporation	|	DMV1234	|	DMV					|	Texas		|	Jun 07, 2016	|	Jun 05, 2022	|	RBS             |	Tulsa - Yale | Trustee       | 444444444 | 1234 Main St, Tulsa, OK 12345 |	
